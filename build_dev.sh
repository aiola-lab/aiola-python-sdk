...
echo -e "${YELLOW}Building aiOla sdk in development mode${NC}"
...
STEP=$((STEP+1))
progress_bar $STEP $TOTAL_STEPS
python -m build libs/speech_to_text/aiola_stt &> /dev/null && pip install -e libs/speech_to_text/aiola_stt"[dev]" &> /dev/null
